...
And =term= will be $\frac{h}{3}A_k y_k$, with $A_k$ being equal to 1 if $k = 1$ or
$k = n$, 4 if $k$ is odd and 2 if $k$ is even
...
#+begin_src scheme :session "1.29" :exports both
(define (cube a) (* a a a))
#+end_src
