...
If we count the =remainder= evaluated explicitly in the trace, and the evaluated
for the if, in total =remainder= has been evaluated 19 times. Since with)
applicative-order, the =remainder= will be evaluated before the =if= statement,
the evaluations due to =if= won't occur, and won't be accumulation of
=remainder= either. This will reduce the evaluation of =remainder= to 4
